...
function sources_mupen64plus() {
    ...
    if isPlatform "gles"; then
        ! isPlatform "videocore" && repos+=('mupen64plus video-glide64mk2')
...
function build_mupen64plus() {
    ...
    if isPlatform "gles"; then
        ! isPlatform "videocore" && md_ret_require+=('mupen64plus-video-glide64mk2/projects/unix/mupen64plus-video-glide64mk2.so')
        if isPlatform "32bit"; then
